...
The gcc.c-torture execution suite is the largest of them and the newest here. It is 1907 programs written over forty years, every one of them a bug report that was once a wrong answer from a released compiler, reduced and kept. `spec/14-target-ladder.md` section 14.1 names it as part of rung 0 of the compiler's target ladder, and the exit criterion there is every one of these programs that does not need an extension nobody has written yet, with the list of the rest checked in and shrinking. This manifest is that list. One hundred and thirty five files are skipped rather than excluded: one hundred and twelve because gcc 16 refuses them itself, mostly on the three permissive rules gcc 14 promoted to errors, and twenty three because they are GNU's nested functions, which this compiler has decided it will not have. The 162 pipeline exclusions point at sixteen issues, and the largest block is twenty two cases, which is what a list looks like after the reasons that were five hundred cases each have been fixed.

The execution list is the other half of that manifest and it is what tamnd/rucc-compat#30 asked for. 314 cases are named across the six optimization levels, in 45 blocks, and at `-O0` 1309 of the 1772 programs pass, 296 are named and 167 are refused by gcc 16 as well. At `-O3` it is 1293, 314 and 165. The programs gcc refuses are not on the list and are not in the skip list either: they are decided by running the reference, so they cannot go stale and nobody has to notice when gcc changes its mind. The whole sweep runs nightly, one job a level, and the six levels are not the same run: the first three blocks are a third of the list, which are inline assembly, an eighty bit float and a vector type, and seventeen more programs are named from `-O1` upward, one of them the other way round, mostly because rucc defines `__OPTIMIZE__` and they then demand optimizations rucc does not do yet.

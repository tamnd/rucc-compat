...
A case can be off the list for two other reasons and the manifest keeps the three apart. A `skip` on the unit is a file the reference compiler refuses as well, so the run has nothing to say about rucc either way. A `[[settled]]` entry is the opposite of that: the reference compiles the file and rucc has decided it will not, which is a statement about this compiler that no issue is ever going to close, so in place of an issue it names the section of the specification where the decision was argued. Those are counted and listed on their own in every report, because a case we have turned down and a case we have not reached yet are not the same news, and an entry parked in the skip array is one nobody ever has to defend.
...
The exclusions work the way the pipeline check's do, in a separate `[[exec-exclude]]` list, and carry three things more. The first is the outcome they cover: an entry that says a case does not build and a case that now builds and prints the wrong answer are not the same entry, so the run fails rather than counting it as covered. The second is an optional `opt`, naming the optimization levels the entry speaks at, for the gaps that are one level's rather than every level's. The third is an optional `route`, naming the build paths it speaks on, for the rarer gaps that are one path's: a relocation a position independent link refuses fails on the path that makes one and passes on the two that hand their output to the reference compiler to link.
...
`--unreached` is the third stage and the stronger form of the same question. Every rule nothing fired has to have an entry in `unreached.toml` giving the reason and the issue that makes it reachable, and every entry has to be about a rule that exists in the set and that nothing fired. So it fails three ways a percentage would hide: a rule nothing fired that no entry explains, an entry on a rule that now fires, and an entry on a pattern the rule set does not contain. That is the rule every exclusion list here is kept under, and it is why the list cannot go stale in either direction.

An entry names the rule by its pattern rather than by the file and line the coverage files carry. A line moves the moment somebody adds a rule above it, and a list keyed on lines would need rewriting on every unrelated change to the rule file, which is how a list stops being read. A pattern moves when the rule is rewritten, and a rewritten rule is exactly the case where somebody should look at the entry again.

Both checks want the whole sweep, and `--unreached` wants it more: one corpus at one level reaches a fraction of the rule set and would report most of the compiler as unexplained.

CI asks both in one job on the nightly schedule rather than in each of the eighteen jobs that run the sweep. Those jobs are eighteen machines that never meet, each writes its own file, and the union of all of them is the only place the whole answer exists. The number is over the two small corpora and the torture suite together, and the torture suite runs nightly, so asking on every commit would be a different and much lower number.

The floor stays alongside the list because the two say different things. The list is about which rules and the floor is about how many, so a change that adds a rule to the set and an entry to the list in the same breath satisfies the list and moves the number. `spec/20-execution-testing.md` section 20.9 in the compiler repository is the design.
...
The pipeline check runs over five corpora. On a linux x86-64 host, c-testsuite is at 220 of 220, tcc tests2 is at 84 of 87, chibicc is at 28 of 29, sqlite is at 2 of 4 and the gcc.c-torture execution suite is at 1715 of 1870, with 155 exclusions covering the rest and pointing at fifteen issues between them. It used to be one case worse, a structure passed in memory to a variadic function, which is a gap that has since been closed, and before that it was sixty worse, all for one reason: rucc claimed to be GCC 4.2.1 by default, so glibc took the branch of `bits/floatn-common.h` that typedefs `_Float32` and the rest over rucc's own keywords, and every case that included a system header stopped on that line. tamnd/rucc#142 raised the claim and the job is no longer advisory. Four of the compiler bugs behind these lists came out of the round trip and the verifier rather than out of a diagnostic, which is to say that no error message would have found them: a stack restore whose saved pointer does not reach it, a flexible array member initializer that makes an image larger than the global it fills, a name that is not ASCII surviving the printer and not the parser, and an object of no size at all printing in a form the reader would not take. All four are fixed.

The chibicc corpus is a different shape from the collections. It is forty one programs written one per language feature by somebody building a C compiler, so the cases sit on the parts that are easy to get wrong and nowhere else, and the overlap with a collection of programs is small. Twelve of them are skipped because gcc 16 refuses them too. Eleven more were skipped on the same claim until somebody ran the experiment it asserted the answer to: chibicc is from 2020 and gcc 14 turned four permissive rules into errors, but `-fpermissive` is the flag gcc put there for exactly that, both compilers read it, and gcc takes all eleven once it is asked to. Of those eleven, three pass both checks, three are excluded against issues rucc has, and five are programs gcc's own build cannot pass either, which the run decides by trying rather than by a comment. Two of the exclusions came off within the week, because a case written one per language feature says exactly what it wants: tamnd/rucc#661 was the obsolete array designator `[3] 7` that `test/initializer.c` writes, and tamnd/rucc#662 was what a `va_end` of something that is not a list is called, which `test/varargs.c` does on the last line of a function. The suite found two things in its first week here. `#pragma once` in the file named on the command line was warned about and then dropped, which broke the one file in the suite that includes itself, and `-E` does not mark an expansion of a macro defined in a system header, which is the register's newest entry.

The gcc.c-torture execution suite is the largest of them and the newest here. It is 1907 programs written over forty years, every one of them a bug report that was once a wrong answer from a released compiler, reduced and kept. `spec/14-target-ladder.md` section 14.1 names it as part of rung 0 of the compiler's target ladder, and the exit criterion there is every one of these programs that does not need an extension nobody has written yet, with the list of the rest checked in and shrinking. This manifest is that list. Seven files are skipped and every one of them is a file gcc 16 refuses itself. Thirty more are settled, which is GNU's nested functions and is the extension this compiler has decided it will not have. There were a hundred and thirty five skips before the harness learned to read the `dg-additional-options` line most of those files write for themselves, which turned a hundred and four claims about gcc into a hundred and four programs that compile. The 155 pipeline exclusions point at fifteen issues, and the largest block is thirty one cases, which is what a list looks like after the reasons that were five hundred cases each have been fixed.

The execution list is the other half of that manifest and it is what tamnd/rucc-compat#30 asked for. 183 cases are named across the six optimization levels, in 32 blocks, and at `-O0` 1549 of the 1870 programs pass, 168 are named and 153 are refused by gcc 16 as well. At `-O3` it is 1532, 183 and 155. The programs gcc refuses are not on the list and are not in the skip list either: they are decided by running the reference, so they cannot go stale and nobody has to notice when gcc changes its mind. The whole sweep runs nightly, one job a level, and the six levels are not the same run: the largest three blocks are over a third of the list between them, which are the programs naming an optimizer option rucc turns down, complex arithmetic and `__int128`, and fifteen cases are named from `-O1` upward rather than everywhere, mostly because rucc defines `__OPTIMIZE__` and they then demand optimizations rucc does not do yet.
...
CI is one Ubuntu image with one libc, one linker and one gcc, and the two things most likely to be wrong about a compiler nobody else has run are the ABI and the link. `scripts/on-hardware.sh` runs the whole of the harness over ssh on machines that are not that image, builds both trees there rather than shipping a binary, and files what comes back under `results/hardware/`, one directory per machine. The host list comes out of the environment or out of a file outside the tree, because whose machines they are is not this repository's business. `execute/pr54937.c` is the case that made the argument: it failed on one machine and passed on another with the same compiler version, because the two gccs were built with different defaults for whether a link makes a position independent executable.

...
The execution check is younger and its number is the one that moves. c-testsuite is at 195 of 220 on a linux x86-64 host, over all three build paths, measured against gcc 16.2.0. It was at 95 a fortnight ago, and two gaps in the back end are the whole of the difference. The first was the address of a name at file scope, which is where every string literal and every use of a global variable starts, and the definitions those names refer to: ninety six cases were waiting on it and eighty two of them run now. The second was that a one bit value, which is what a comparison produces, was not a width the lowering rules were written at, so a comparison used as a number rather than as a branch stopped the file. Twenty cases were waiting on that and eighteen of them run now, the other two having a second gap behind the first. What is left is twenty five cases and eight issues, and no block is larger than seven. The chibicc suite is at none of its eighteen for a reason worth writing down: every case there is built with one helper file, that file has a variadic function in it, and nothing lowers `va_start`, so one gap holds the whole corpus and the first case to run will be all of them.
